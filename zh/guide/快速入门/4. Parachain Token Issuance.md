...
<font color=red>注意：</font>在发行token的过程成每个操作都需要花费0.0001个BTY，请保证用户地址中有足够的BTY，避免因为余额不足而导致token发行失败。
## 检查平行链节点
...
```  
--conf：配置文件名称，-c：固定填"token-blacklist"，-o：add（表示添加），-v：黑名单名称，比如BTY，--rpc_laddr：平行链节点监听地址，--paraName：对应平行链的名称。  
2. 交易签名。
```
./chain33-cli-linux-amd64 --rpc_laddr="http://localhost:8901" --paraName="user.p.devtest." wallet sign -k 3990969DF92A5914F7B71EEB9A4E58D6E255F32BF042FEA5318FC8B3D50EE6E8 -d "上一步生成的数据"  
```  
