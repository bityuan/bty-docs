...
在平行链的使用场景中，可以发送不收手续费的交易，具体步骤为：
构造交易 -> **平行链交易包装** -> 交易签名 -> 发送交易

注意：交易签名步骤里需要注意一点，参数index需填2。
